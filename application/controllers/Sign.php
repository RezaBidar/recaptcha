...
class Sign extends My_Controller{
	...
	public function in()
	{
		if($this->m_user->loggedin()) redirect('panel/dashboard');
		//set form validation
		$rules = array(
		...
		if($this->form_validation->run() == TRUE)
		{
			...
			{
				$this->session->set_flashdata('error', 'User Or Password is incorrect');
			}
		}
		else
		{
		...
		$this->load->view('layouts/modal_layout', $this->data);
	}
	...
	{
		$this->data['content_view'] = 'documentation' ;
		$this->load->view('layouts/modal_layout', $this->data);	
	}
}
